...
public class CustomCommitHandler implements CommitHandler {
    ...
    public CustomCommitHandler(final String transactionId, final Network network, final CommitStrategy strategy, final ActorRef actorRef) {
    ...
    private void processStrategyResult(final CommitStrategy.Result strategyResult) {
        if (strategyResult == CommitStrategy.Result.SUCCESS) {
            ...
            transactionResult.setTransactionError(new TransactionError("CFail", "Failed commit strategy "+strategy.toString(),Instant.now()));
